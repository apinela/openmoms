...
import com.openmoms.config.KafkaProperties;
...
class OpenMomsKafkaResourceIT {
    ...
    void setup() {
        ...
        OpenMomsKafkaResource kafkaResource = new OpenMomsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/open-moms-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/open-moms-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
